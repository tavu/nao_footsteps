...
{
Footstep::Footstep(double x, double y, double theta, float w, double cell_size,
                   int num_angle_bins, int max_hash_size,double dif_angle_cost)
  ...
  ivDiscPredecessorRight(num_angle_bins)
{
    ...
    int angleBinsHalf=ivNumAngleBins/2;
    int angleBin;
    if(ivTheta > angleBinsHalf)
        angleBin=ivNumAngleBins - ivTheta;
    else
        angleBin=ivTheta;
//     ivCost =1.0 + angleBin * 2.0;
    ivCost = w + angleBin * dif_angle_cost;
//     ivCost = w;
//     ROS_ERROR("x:%f y:%f th:%f a:%d, c:%f", x,y,theta,angleBin, ivCost);
}

double Footstep::getCost() const
{
    return ivCost;
}
