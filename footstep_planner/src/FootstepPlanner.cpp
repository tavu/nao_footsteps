...
//   nh_private.param("allocated_time", ivMaxSearchTime, 7.0);
  ...
  XmlRpc::XmlRpcValue footsteps_w;
  ...
  nh_private.getParam("footsteps/w", footsteps_w);
  ...
  if (footsteps_w.getType() != XmlRpc::XmlRpcValue::TypeArray)
    ROS_ERROR("Error reading footsteps/w from config file.");
  int size_x = footsteps_x.size();
  int size_y = footsteps_y.size();
  int size_t = footsteps_theta.size();
  int size_w = footsteps_w.size();
  if (size_x != size_y || size_x != size_t || size_x != size_w)
  {
    ROS_ERROR("Footstep parameterization has different sizes for x/y/theta/w. "
              "Exit!");
    exit(2);
  }
  // create footstep set
  ...
  for(int i=0; i < footsteps_x.size(); ++i)
  {
    ...
    double w = (double)footsteps_w[i];

    Footstep f(x, y, theta, (float)w,
  ...
  ivEnvironmentParams.max_forward_step=max_x;
//   ivEnvironmentParams.max_step_width = max_x;

  // initialize the heuristic
  boost::shared_ptr<Heuristic> h;
  ROS_INFO("FootstepPlanner heuristic:%s",heuristic_type.c_str());
  ...
  else if(heuristic_type == "EuclStepCostHeuristic2")
  {
//       EUCLIDEAN_STEPCOST2
    h.reset(
        new EuclidianStepCost2(ivEnvironmentParams.cell_size,
                                  ivEnvironmentParams.num_angle_bins,
    ...
    ROS_INFO("FootstepPlanner heuristic: euclidean distance with step costs 2");
  }
  ...
  else if (heuristic_type == "StepCostHeuristic")
  {
    // for heuristic inflation
    double foot_incircle =
      std::min((ivEnvironmentParams.footsize_x / 2.0 -
    ...
    h.reset(
        new StepCostHeuristic(ivEnvironmentParams.cell_size,
                              ivEnvironmentParams.num_angle_bins,
                              ivEnvironmentParams.step_cost,
                              diff_angle_cost,
                              max_step_width,
                              foot_incircle));
    ROS_INFO("FootstepPlanner heuristic: 2D step costs");
  }
  else
  {
...
FootstepPlanner::run()
{
  ...
  ivStartId=ivPlannerEnvironmentPtr->updateStart(ivStartFootLeft, ivStartFootRight).first;
  ivEndId=ivPlannerEnvironmentPtr->updateGoal(ivGoalFootLeft, ivGoalFootRight).first;
  ...
  int hvalue;
  if(ivEnvironmentParams.forward_search ||true)
  {
    hvalue=ivPlannerEnvironmentPtr->GetFromToHeuristic(ivStartId,ivEndId);
  }
  else
  {
    hvalue=ivPlannerEnvironmentPtr->GetFromToHeuristic(ivEndId,ivStartId);
  }
  
  ROS_INFO("Start planning (max time: %f, initial eps: %f (%f) heuristic: %d )\n",
                             ...
                             humanoid_nav_msgs::PlanFootstepsBetweenFeet::Response &resp)
{
  ...
  ivMaxSearchTime=req.planning_time;
  ivSearchUntilFirstSolution=req.until_first_solution;
  ivInitialEpsilon=req.initial_eps;
