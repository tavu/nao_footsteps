...
: DiscreteSpaceInformation(),
  ...
  ivMaxForwStep(params.max_forward_step)
{
...
int 
FootstepPlannerEnvironment::costFromFootstep(const Footstep &f) const
{    
//     return ivStepCost;
//     return  int(cvMmScale*f.getCost() );
   int cost=int(ivStepCost*f.getCost() );
//    ROS_ERROR("c:%d",cost);
   return cost;
}
...
//   return ivStepCost * dist / ivMaxForwStep;
}
...
FootstepPlannerEnvironment::updateMap(gridmap_2d::GridMap2DPtr map)
{
  ...
  else if(ivHeuristicConstPtr->getHeuristicType() == Heuristic::STEP_COST)
  {
    boost::shared_ptr<StepCostHeuristic> h =
        boost::dynamic_pointer_cast<StepCostHeuristic>(
            ivHeuristicConstPtr);
    h->updateMap(map);
...
FootstepPlannerEnvironment::updateHeuristicValues()
{
  ...
  if (!ivHeuristicExpired) {
    ROS_DEBUG("Heuristic is not expired. No need for update.");
    return;
  }
...
// int FootstepPlannerEnvironment::getHvalue(int id1 ,int id2) const
// {
//     const PlanningState* s1=ivStateId2State[id1];
//     const PlanningState* s2=ivStateId2State[id2];
//     return GetFromToHeuristic(s1,s2);
// }
                                     ...
                                     std::vector<int> *CostV)
{
  ...
  PredIDV->reserve(ivFootstepSet.size()+1);
  CostV->reserve(ivFootstepSet.size()+1);
  std::vector<Footstep>::const_iterator footstep_set_iter;
  bool addGoalSteps=true;
  for(footstep_set_iter = ivFootstepSet.begin();
      footstep_set_iter != ivFootstepSet.end();
      ++footstep_set_iter)
  {
...
//     int cost = stepCost(*current, *predecessor_hash);    
    int cost = costFromFootstep(*footstep_set_iter);
    PredIDV->push_back(predecessor_hash->getId());
    CostV->push_back(cost);
    if(predecessor_hash->getId()==ivIdStartFootLeft || predecessor_hash->getId()==ivIdStartFootRight)
        addGoalSteps=false;
  }
  
  if (closeToStart(*current) && addGoalSteps)
  {
...
//   return GetFromToHeuristic(stateID, ivIdStartFootLeft);
  return GetFromToHeuristic(ivIdStartFootLeft,stateID);
}
                                     ...
                                     std::vector<int> *CostV)
{
  ...
  const PlanningState* goal_left = ivStateId2State[ivIdGoalFootLeft];
  const PlanningState* start_left = ivStateId2State[ivIdStartFootLeft];
  double hh=ivHeuristicConstPtr->getHValue(*start_left,*goal_left);
//  ROS_ERROR("Start heuristic:%f\n",hh);
  ...
  SuccIDV->reserve(ivFootstepSet.size()+1);
  CostV->reserve(ivFootstepSet.size()+1);
  bool addGoalSteps=true;
  std::vector<Footstep>::const_iterator footstep_set_iter;
  for(footstep_set_iter = ivFootstepSet.begin();
...
//     int cost = stepCost(*current, *successor_hash_entry);
    ...
    CostV->push_back(cost);
    if(successor_hash_entry->getId()==ivIdStartFootLeft || successor_hash_entry->getId()==ivIdStartFootRight)
        addGoalSteps=false;
  }
  
  if (closeToGoal(*current) &&addGoalSteps)
  {
    int goal_id;
  ...
  for (int step_y = ivMaxInvFootstepY; step_y <= ivMaxFootstepY; ++step_y)
  {
    for (int step_x = ivMaxInvFootstepX; step_x <= ivMaxFootstepX; ++step_x)
    {
           ...
           ++step_theta)
      {
        ...
        Footstep step(cont_step_x, cont_step_y, cont_step_theta,1,
                      ivCellSize, ivNumAngleBins, ivHashTableSize,2.0);
        if (ivForwardSearch)
        {
